...
class TwoFactorAuthenticationStatus
{
    ...
    public function handle(Request $request, Closure $next, string $status = 'enabled')
    {
        ...
        if ($status === 'enabled' && $request->routeIs('admin.*') && ! config('boilerplate.access.user.admin_requires_2fa')) {
            return $next($request);
        }
        ...
        if (
            ...
            $routePrefix = $request->routeIs('admin.*') ? 'admin' : 'frontend';
            return redirect()->route($routePrefix.'.auth.account.2fa.create')->withFlashDanger(__('Two-factor Authentication must be :status to view this page.', ['status' => $status]));
        }
