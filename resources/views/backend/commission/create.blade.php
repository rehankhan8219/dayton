...
@section('title', __('Create Commission'))
@section('page-title', __('Create Commission'))

@section('content')
    <x-forms.post :action="route('admin.commission.store')">
        <x-backend.card>
            <x-slot name="body">
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Commission')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#user_id').change(function() {
                ...
                if ($('#user_id').val()) {
                    $.ajax({
                        url: '{{ route('admin.member.broker.fetch', '##MEMBER_ID##') }}'.replace(
