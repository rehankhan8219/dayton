...
@section('title', __('Update Pay Now Account'))

@section('content')
    <x-forms.patch :action="route('admin.payaccount.update', $payAccount)" enctype="multipart/form-data">
        @php(html()->model($payAccount))
        <x-backend.card>
            <x-slot name="body">
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Update Pay Account')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.patch>
@endsection
